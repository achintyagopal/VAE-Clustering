...
class VAE(nn.Module):
    def __init__(self):
        ...
        # self.seq = nn.Sequential(
        #     # input is 1 x 28 x 28
        #     nn.Conv2d(1, 128, 4, 4, 1, bias=False),
        #     nn.LeakyReLU(0.2, inplace=True),
        #     nn.BatchNorm2d(128)
        # )
        ...
        self.fc23 = nn.Linear(1024, 10)

        # have z vector
        self.fc3 = nn.Linear(self.encoder_size(), 1024)

        self.fc4 = nn.Linear(1024, 128*7*7)
        # need to resize (batch,128,7,7)
...
def loss_function(recon_xs, x, mu, logvar, categorical, y_class):
    BCE = 0
...
def train(epoch):
    ...
    for batch_idx, (data, y_class) in enumerate(train_loader):
        ...
        loss = loss_function(total_batch, data, mu, logvar, categorical, y_class)
...
def test(epoch):
    ...
    for batch_idx, (data, z_class) in enumerate(test_loader):
        ...
        test_loss += loss_function([recon_batch], data, mu, logvar, categorical, y_class)
