...
parser.add_argument("--save-model", default="cnn_mnist", type=str,
        help="The file containing already trained model.")
...
class VAE(nn.Module):
    ...
    def forward(self, x):
        y = self.seq(x)
        return self.softmax(self.fc(y.view(-1, 6272)))
...
def loss_function(y_class, y_pred):
    ...
    # return torch.sum(-Variable(torch.from_numpy(y_class).type(torch.FloatTensor)).mul(y_pred.log()))
